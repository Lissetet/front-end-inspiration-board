...
const Footer = () => {
  return (
    <footer className="bg-black bg-opacity-30 py-4 px-8 mt-20 bottom-0">
        ...
        <article className="flex flex-col lg:w-1/2">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {teamMembers.map((member, index) => (
              <li key={index}>
                <a
                  href={member.porfolioSite || member.githubUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="flex gap-4 hover:underline hover:text-primary"
                >
                  <img
                    src={member.imgUrl}
      ...
      <section
        className="border-t max-w-7xl mx-auto mt-4 pt-2 flex justify-center gap-3"
      >
